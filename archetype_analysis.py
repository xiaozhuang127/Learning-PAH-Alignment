...
class AABase_3(AABase):
    ...
    def __init__(
        ...
        if generator is not None:
            ...
            self.generator.manual_seed(generator.initial_seed())
        else:
            self.generator = torch.Generator(device='cpu')

    def _init_A(self, X):
        A = torch.zeros((X.shape[0], self.n_archetypes), dtype=X.dtype, device=self.device)
        ind = torch.randint(0, self.n_archetypes, (X.shape[0],), generator=self.generator, device='cpu').to(self.device)
    ...
    def _init_B(self, X):
        ...
        # 把 X 移到 CPU 进行随机索引选取
        cpu_X = X.cpu()
        ind = self.init_c_(cpu_X, self.n_archetypes, generator=self.generator, kwargs=self.init_kwargs)
    ...
    def _compute_archetypes(self, X):
        ...
        self.archetypes_ = self.archetypes_.to(self.device)

    def _loss(self, X):
        X = X.to(self.device)
        self.archetypes_ = self.archetypes_.to(self.device)
        X_hat = torch.matmul(self.A_, self.archetypes_)
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        if self.save_init:
            self.archetypes_init_ = self.archetypes_.clone()
...
def _autogd_init_A(self, X):
    ...
    self.A_opt_ = torch.asarray(A_, requires_grad=True, device=self.device)
...
def _autogd_init_B(self, X):
    ...
    self.B_opt_ = torch.asarray(B_, requires_grad=True, device=self.device)
...
def _autogd_optim_A(self, X):
    # # 打印损失计算前 A_opt_ 的形状
    # print("Before loss calculation, A_opt_ shape:", self.A_opt_.shape)
    # # 打印损失计算前 A_opt_ 的值
    # print("Before loss calculation, A_opt_ value:", self.A_opt_)
    loss = self._loss(X)
    self.optimizer_A.zero_grad()
    loss.backward(retain_graph=True)
    # # 打印梯度更新前 A_opt_ 的梯度
    # print("Before optimizer step, A_opt_ grad:", self.A_opt_.grad)
    self.optimizer_A.step()
    # # 打印梯度更新后 A_opt_ 的值
    # print("After optimizer step, A_opt_ value:", self.A_opt_)
    A_opt_softmax = torch.softmax(self.A_opt_, dim=1)
    return A_opt_softmax
...
def _autogd_optim_B(self, X):
    ...
    B_opt_softmax = torch.softmax(self.B_opt_, dim=1)
    return B_opt_softmax
